...
<x-card title="Internship Map" subtitle="Overview of registered internship point and attendance markers.">
        ...
        <div class="h-[70vh] rounded-xl border border-gray-200 bg-gradient-to-br from-gray-100 to-white p-4">
            <div class="relative flex h-full items-center justify-center rounded-xl border border-dashed border-gray-300 bg-white text-center">
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">{{ $locations->count() }} location marker(s) ready to render.</p>
                </div>
    ...
    @else
        <div class="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-10 text-center">
            <p class="text-sm font-semibold text-gray-700">No map data available</p>
            <p class="mt-1 text-sm text-gray-500">Add at least one location to enable map markers.</p>
            <a href="{{ route('user.locations.index') }}" class="mt-4 inline-flex rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">Go to Locations</a>
        </div>
    @endif
</x-card>
@endsection
